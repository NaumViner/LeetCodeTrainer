create or replace function public.start_mock_interview_v2(
  p_problem_id uuid,
...
begin
  ...
  if p_interview_language not in ('english', 'hebrew') then
  ...
  if p_selection_mode <> 'coverage' then
    raise exception 'New interviews use coverage selection.' using errcode = '42501';
  end if;
...
-- Assessment is optional and can use the default profile without onboarding.
do $$
declare definition text;
begin
  definition := pg_get_functiondef('public.begin_diagnostic(jsonb)'::regprocedure);
  definition := replace(definition, 'where id = current_user_id and onboarding_completed', 'where id = current_user_id');
  execute definition;
end;
$$;
