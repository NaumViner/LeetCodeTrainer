...
Before a hosted release, follow [deployment.md](deployment.md): dry-run and apply migrations to the intended Supabase project, confirm RLS remains enabled, rotate any keys exposed outside approved secret storage, configure exact production redirect URLs, and require green CI. After release, `/api/health` verifies public database readiness without returning credentials, queries, or internal error details.
