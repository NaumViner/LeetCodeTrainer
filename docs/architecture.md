...
Post-interview evaluation begins at a separate server-only evidence boundary. A strict, versioned package combines only learner-owned interview state, bounded phase notes, the latest code snapshot, final transcript turns, safe provider metadata, timing boundaries, and learner outcome. It excludes provider call IDs, secrets, external URLs, and raw private tests. A distinct first-party question registry separates legally usable prompt/invariant context from private evaluator tests; while that registry is empty, semantic correctness evidence is explicitly unsupported rather than inferred from a catalog title.

The optional learning coach sits behind a provider interface that is separate from the realtime interviewer. Gemini structured output is the default adapter and OpenAI Responses remains available; both use strict JSON schemas, Zod validation, an eight-second timeout, and one safe retry. Practice actions assemble the minimum learner context server-side. Progressive hints, pattern and complexity feedback, attempt analysis, and review-card drafts persist through one private interaction ledger; review cards reappear as active-recall prompts on the next attempt. Built-in deterministic behavior remains the fallback, so neither missing configuration nor provider failure blocks practice.

The realtime interviewer has its own provider boundary and is disabled unless its server-only flag and credentials are present. Each mock interview persists a `beginner` or `faang_tough` interviewer level, and the same level-specific instruction builder feeds every provider connection. Gemini Live is the default: an authenticated Route Handler verifies interview ownership and issues a one-use ephemeral token, then the browser streams PCM audio directly to Gemini with transcription, interruption, and session resumption. The OpenAI alternative uses an authenticated server-side SDP proxy and browser WebRTC. In both paths the permanent provider key never reaches browser JavaScript. Typed turns, phase context, and code snapshots share the same provider-independent interface. Narrow database functions persist completed transcript/context events and lifecycle state under forced RLS. The text-only mock interview remains fully operational when realtime is disabled or disconnected.

The interface uses semantic color tokens with an early theme bootstrap for System, Light, and Dark preferences. Navigation, forms, recovery states, and analytics preserve keyboard and screen-reader semantics. Progress visuals are server-rendered SVG and CSS with adjacent exact values. The optional realtime client is dynamically loaded only for voice-enabled sessions, keeping the default text interview bundle smaller. See [product-polish.md](product-polish.md) for the Phase 14 review.
