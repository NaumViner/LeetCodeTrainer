...
The first half of Phase 12 implements the complete provider boundary and two learner-facing capabilities: AI-enhanced progressive hints and pattern analysis. Phase 12B will add complexity feedback, post-attempt analysis, and review-card drafting through the same interface.
...
When the feature is disabled, existing deterministic progressive hints continue unchanged. When a configured provider fails or returns invalid output, hints and pattern coaching use bounded deterministic fallbacks. A private reservation is created before each provider call. The database permits 20 requests per learner in a rolling 24-hour window and records model, status, structured response, and reported token usage.
